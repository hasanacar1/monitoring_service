...
import libvirt
import math
from common import client_utils
import os
import re
import stat
import subprocess
...
class LibvirtCheck(AgentCheck):
    ...
    def __init__(self, name, init_config, agent_config, instances=None):
        ...
        self.instance_cache_file = "common/libvirt_cache/libvirt_instances.json"
        self.metric_cache_file = "common/libvirt_cache/libvirt_metrics.json"
    ...
    def _test_vm_probation(self, created):
        ...
        probation_time = 300 - created_sec
    ...
    def _update_instance_cache(self):
        ...
        print("UPDATE_1")
        session = client_utils.get_session()
        nova_client = client_utils.get_nova_client()
        ...
        ping_check = "sudo /bin/ip netns exec NAMESPACE /bin/ping -n -c1 -w1 -q"
        if ping_check:
            nu = client_utils.get_neutron_client()
        ...
        #
        tenants = []
        keystone_client = client_utils.get_keystone_client()
        ...
        for instance in instances:
            ...
            tenant_name = client_utils.get_tenant_name(tenants, instance.tenant_id)
            if tenant_name:
                id_cache[inst_name]['tenant_name'] = tenant_name

            for config_var in ['metadata', 'customer_metadata']:
                for metadata in ['scale_group']:
        ...
        try:
            ...
            if stat.S_IMODE(os.stat(self.instance_cache_file).st_mode) != 0o777:
                os.chmod(self.instance_cache_file, 0o777)
        except IOError as e:
    ...
    def _load_instance_cache(self):
        ...
        try:
            with open(self.instance_cache_file, 'r') as cache_json:
                ...
                nova_refresh = 14400
                if nova_refresh is not None:
                    time_diff = time.time() - instance_cache['last_update']
                    if time_diff > nova_refresh:
    ...
    def _update_metric_cache(self, metric_cache, run_time):
        ...
        try:
            ...
            if stat.S_IMODE(os.stat(self.metric_cache_file).st_mode) != 0o777:
                os.chmod(self.metric_cache_file, 0o777)
        except IOError as e:
    ...
    def _inspect_disks(
        ...
        if vm_extended_disks_check_enable:
    ...
    def _run_ping(self, dims_customer, dims_operations, inst_name, instance_cache, net):
        ...
        ping_check = "sudo /bin/ip netns exec NAMESPACE /bin/ping -n -c1 -w1 -q"
        ping_cmd = ping_check.replace('NAMESPACE', net['namespace']).split()
    ...
    def check(self, instance=None):
        ...
        for inst in insp._get_connection().listAllDomains():
            ...
            ping_check = "sudo /bin/ip netns exec NAMESPACE /bin/ping -n -c1 -w1 -q"
            if vm_ping_check_enable and ping_check and 'network' in instance_cache.get(inst_name):
    ...
    def _get_this_host_aggregate(self, nova_client):
        ...
        host_agg_cfg_re = host_aggregate_re
