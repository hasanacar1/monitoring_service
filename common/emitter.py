...
def http_emitter(message, url):
    ...
    try:
        ...
        try:
            ...
            print("Forwarder at {0} is down or not responding... Error is {1} Please restart the monasca-agent.".format(url, repr(exc)))
        finally:
            if response:
                print(response)
                response.close()
