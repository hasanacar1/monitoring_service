...
def transform(metrics, tenant_id, region):
    ...
    if isinstance(metrics, list):
        ...
        for metric in metrics:
            ...
            transformed_metrics.append(as_json(transformed_metric))
        return transformed_metrics
    else:
        transformed_metric['metric'] = metrics
        return [as_json(transformed_metric)]
...
class MetricsV2API(object):
    def __init__(self):
        ...
        print('Initializing MetricsV2API!')
...
class Metrics(MetricsV2API):
    def __init__(self):
        try:
            super(Metrics, self).__init__()
            self._region = 'RegionOne'
            self._message_queue = kafka_publisher.KafkaPublisher('metrics')
            #self._metrics_repo = monasca_api.common.repositories.influxdb.metrics_repository:MetricsRepository()
            queue_buffering_max_messages = 1000
            self._batch_size = queue_buffering_max_messages
    ...
    def on_post(self, req, res):
        ...
        # print(tenant_id)
        # print("*************")
        tenant_id = "37550090964b4f0dbd9df45a174b1d1d"
        transformed_metrics = transform(
