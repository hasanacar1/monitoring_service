...
class MonascaAPI(object):
    ...
    def __init__(self, config):
        ...
        self._max_buffer_size = int(config['Api']['max_buffer_size'])
        self._max_batch_size = int(config['Api']['max_batch_size'])
        self._max_measurement_buffer_size = int(
            config['Api']['max_measurement_buffer_size'])
        ...
        self.backlog_send_rate = int(config['Api']['backlog_send_rate'])
        if self._max_buffer_size > -1:
            self.message_queue = collections.deque(maxlen=self._max_buffer_size)
        else:
            self.message_queue = collections.deque()
        self.write_timeout = int(config['Api']['write_timeout'])
        # 'amplifier' is completely optional and may not exist in the config
        try:
            self.amplifier = int(config['Api']['amplifier'])
        except KeyError:
            self.amplifier = None
    ...
    def _post(self, measurements, tenant=None):
        ...
        self._send_message(measurements)
    ...
    def _send_message(self, measurements):
        try:
            #self._mon_client.metrics.create(**kwargs)
            base_url = "http://127.0.0.1:8071/v2.0/metrics"
            ...
            #print(data)
            response = requests.request("POST", base_url, headers=headers, data=data, verify=False)
            print(response)
