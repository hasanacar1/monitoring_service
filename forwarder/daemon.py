...
class Forwarder(tornado.web.Application):
    def __init__(self, port, agent_config, skip_ssl_validation=False,
        ...
        self._non_local_traffic = False

        self.skip_ssl_validation = False
    ...
    def _bind_http_server(self, http_server):
        try:
            ...
            print("Socket error {}. Is another application listening on the same port ? Exiting".format(e))
            sys.exit(1)
        except Exception:
            #log.exception("Uncaught exception. Forwarder is exiting.")
            print("Uncaught exception. Forwarder is exiting.")
            sys.exit(1)
    ...
    def flush(self):
        ...
        if len(message_batch) >= MIN_BATCH_SIZE or self._unflushed_iterations >= MAX_FLUSH_ATTEMPTS:
            # print(message_batch[0]['measurement']['name'])
            # print(message_batch[160]['measurement']['name'])
            #print(message_batch)
            self._post_metrics()
            message_batch = []
            #self._post_metrics()
...
def init_forwarder(skip_ssl_validation=False, use_simple_http_client=False):
    ...
    agent_config = {'Main': {'check_freq': 15,
        ...
        'Logging': {'disable_file_logging': False,
                    ...
                    'syslog_host': None,
                    'syslog_port': None}}
    port = agent_config['Main']['listen_port']
    if port is None:
        port = 17123
    else:
        port = int(port)
